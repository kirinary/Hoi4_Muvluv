...
NDiplomacy = {
	...
	TENSION_TIME_SCALE_START_DATE = "1973.1.1.6",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	...
	JOIN_FACTION_LIMIT_CHANGE_AT_WAR = -0.4,			-- if in defensive war this or your modifier is counted as limit to join factions (so if you have 80% join limit this now means you can join at 50%)
...
NMilitary = {
	...
	NUKE_MAX_DAMAGE_PERCENT = 1.0,					-- Minimum damage from nukes as a percentage of current strength/organisation
...
NAir = {
	...
	COMBAT_SITUATION_WIN_CHANCE_FROM_STATS = 0.2, 		-- How much good stats difference (speed+agility) affects the winning chance (situation win = give a hit, not necessary kill).( Naval air combat only )
	COMBAT_SITUATION_WIN_CHANCE_FROM_GANG = 0.18, 		-- How much bonus gives the airplanes amount advantage (2vs1), to winning the situation.( Naval air combat only )
	...
	ACCIDENT_CHANCE_BASE = 0.5,							-- Base chance % (0 - 100) for accident to happen. Reduced with higher reliability stat.
...
NNavy = {
	...
	COMBAT_EVASION_TO_HIT_CHANCE_TORPEDO_MULT = 2.0,				-- the above evasion hit chance is multiplied by 400% if shooting with torpedos. Torpedoes are slow, so evasion matters more.
